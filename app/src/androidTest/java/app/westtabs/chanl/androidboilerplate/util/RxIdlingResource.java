...
public class RxIdlingResource implements IdlingResource {
    ...
    public void incrementActiveSubscriptionsCount() {
        ...
        Log.i(TAG, "incrementActiveSubscriptionsCount: Active subscriptions count increased to" + count);
    }

    public void decrementActiveSubscriptionsCount() {
        int count = mActiveSubscriptionsCount.decrementAndGet();
        Log.i(TAG, "decrementActiveSubscriptionsCount: Active subscriptions count decreased to " + count);
        if (isIdleNow()) {
            Log.i(TAG, "decrementActiveSubscriptionsCount: There is no active subscriptions, transitioning to Idle");
